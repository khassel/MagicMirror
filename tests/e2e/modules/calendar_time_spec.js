...
describe("Calendar module", () => {
	...
	const doTest = async (cssClass) => {
		...
		const elem = await helpers.waitForElement(`.calendar .module-content .event${cssClass}`);
	...
	const doTestCount = async () => {
		await helpers.getDocument();
		const elem = await helpers.waitForAllElements(".calendar .event");
		expect(elem).not.toBeNull();
		const l = await elem.length;
		return l;
	};
	...
	describe("Test css classes", () => {
		it("has css class dayBeforeYesterday", async () => {
			await helpers.startApplication("tests/configs/modules/calendar/custom.js", "2030-01-03T12:30:00.000Z");
			await expect(doTest(".dayBeforeYesterday")).resolves.toBe(true);
		});

		it("has css class yesterday", async () => {
			await helpers.startApplication("tests/configs/modules/calendar/custom.js", "2030-01-02T12:30:00.000Z");
			await expect(doTest(".yesterday")).resolves.toBe(true);
		});

		it("has css class today", async () => {
			await helpers.startApplication("tests/configs/modules/calendar/custom.js", "2030-01-01T12:30:00.000Z");
			await expect(doTest(".today")).resolves.toBe(true);
		});

		it("has css class tomorrow", async () => {
			await helpers.startApplication("tests/configs/modules/calendar/custom.js", "2029-12-31T12:30:00.000Z");
			await expect(doTest(".tomorrow")).resolves.toBe(true);
		});

		it("has css class dayAfterTomorrow", async () => {
			await helpers.startApplication("tests/configs/modules/calendar/custom.js", "2029-12-30T12:30:00.000Z");
	...
	describe("Events from multiple calendars", () => {
		it("should show multiple events with the same title and start time from different calendars", async () => {
			await helpers.startApplication("tests/configs/modules/calendar/show-duplicates-in-calendar.js", "2024-09-15T12:30:00.000Z");
